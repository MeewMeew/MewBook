  ...
  <FriendLayout>
    <template #sidebar>
      <div class="flex flex-col justify-between select-none">
        <div class="transition-all flex justify-start items-center space-x-3 pb-4 border-b-2 w-full drop-shadow-sm">
        ...
        <div class="h-[89vh] overflow-y-scroll nice-scrollbar">
          <div class="h-full flex flex-col w-full items-start rounded-lg transition-all">
            ...
            <div
              ...
              v-for="(user, index) in frequest" :key="index">
              <router-link :to="{ name: 'frequest-view', params: { id: user.id } }"
                class="relative w-14 h-14 cursor-pointer mr-2">
                <Avatar :image="user.photoURL" :pt="{ root: 'w-16 h-16', image: 'rounded-full w-full h-full' }" />
              </router-link>
              <div class="flex flex-col w-full space-y-1">
                <span class="text-[#050505] text-lg font-medium truncate">{{ user.displayName }}</span>
                <div class="flex flex-row justify-between items-center w-full space-x-2" v-if="!is.friend[user.id.toString()] && !is.accept[user.id.toString()] && !is.reject[user.id.toString()]">
                  <Button aria-label="Chấp nhận" class="space-x-2 w-1/2 justify-center py-2" size="small"
                    :loading="loading.accept[user.id.toString()]" @click="acceptRequest(user.id)">
                    <i v-if="loading.accept[user.id.toString()]" class="pi pi-spin pi-spinner"></i>
                    <span class="font-medium text-[16px]">Chấp nhận</span>
                  </Button>
                  <Button aria-label="Từ chối"
                    class="space-x-2 w-1/2 justify-center bg-mb-gray hover:bg-mb-gray-h border-mb-gray" size="small"
                    severity="secondary" :loading="loading.reject[user.id.toString()]" @click="() => rejectRequest(user.id)">
                    <i v-if="loading.reject[user.id.toString()]" class="pi pi-spin pi-spinner text-mb-gray-2"></i>
                    <span class="font-medium text-[16px] text-center text-mb-gray-2">Từ chối</span>
                  </Button>
                </div>
                <div v-else>
                  <span class="text-[#65676B] text-md font-medium truncate" v-if="is.friend[user.id.toString()]">Đã là bạn bè</span>
                  <span class="text-[#65676B] text-md font-medium truncate" v-else-if="is.accept[user.id.toString()]">Đã chấp nhận lời mời</span>
                  <span class="text-[#65676B] text-md font-medium truncate" v-else-if="is.reject[user.id.toString()]">Đã từ chối lời mời</span>
                </div>
              </div>
            </div>
          </div>
    ...
    <template #view>
      ...
      <div v-else class="h-full w-full justify-center items-center flex select-none">
        <div class="flex flex-col justify-center items-center space-y-4">
          ...
          <span class="text-2xl font-medium text-mb-gray-2">Chọn một người bạn để xem trước trang cá nhân.</span>
        </div>
      </div>
    </template>
  </FriendLayout>
