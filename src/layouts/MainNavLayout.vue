...
const props = defineProps({
  ...
  hide: {
    type: String as PropType<'messenger' | 'none'>,
    default: 'none',
    required: false
  }
})
...
<template>
  ...
  >
    <div class="flex items-center justify-start w-1/6">
      ...
      <div
        class="flex relative bg-[#EFF2F5] items-center justify-center lg:rounded-[10vw] lg:w-2/3 w-[39px] rounded-full cursor-pointer"
      >
      ...
      <router-link class="block md:hidden" :to="{ name: 'bookmarks' }">
        ...
        <div
          v-if="select === 'bookmarks'"
          class="w-[90%] h-[4px] bg-mb-blue mx-auto rounded-full -mb-1"
        ></div>
      </router-link>
    </div>
    ...
    <div class="hidden w-2/5 select-none md:flex justify-start md:justify-center items-center">
      ...
      <router-link :to="{ name: 'friends' }" v-tooltip="{ content: 'Bạn bề' }">
        <IconButton>
      ...
      <router-link :to="{ name: 'dashboard' }" v-tooltip="{ content: 'Xem video' }">
        <IconButton>
      ...
      <router-link :to="{ name: 'dashboard' }" v-tooltip="{ content: 'Place market' }">
        <IconButton>
          <PlaceMarket :fill="select === 'placemarket'" />
        </IconButton>
        <div
          v-if="select === 'placemarket'"
          class="w-[90%] h-[4px] bg-mb-blue mx-auto rounded-full -mb-1"
        ></div>
      </router-link>
      <router-link :to="{ name: 'dashboard' }" v-tooltip="{ content: 'Nhóm' }">
        <IconButton>
        ...
        >
          ...
          >
            <div class="flex items-center gap-3 hover:bg-gray-200 p-2 rounded-lg">
              <img
                class="avatar ml-1 max-w-[35px] max-h-[35px] cursor-pointer"
                :src="cuser.photoURL"
              />
          ...
          <router-link :to="{ name: 'logout' }" class="w-full">
            <div
              class="flex items-center gap-3 hover:bg-gray-200 px-2 py-2.5 rounded-lg cursor-pointer"
            >
