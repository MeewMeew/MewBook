...
<template>
  <div class="fixed z-50 top-0 left-0 w-full h-full bg-white bg-opacity-70">
    <div class="grid h-screen place-items-center p-4">
      <div class="bg-white w-full max-w-[500px] mx-auto shadow-xl rounded-xl">
        <div class="flex items-center relative my-3.5 mx-1">
          <div class="text-[22px] font-bold w-full text-center">Tạo bài viết</div>
          <i @click="isPostOverlay = false"
        ...
        <div class="border-t border-t-gray-300" v-if="cuser">
          <div class="px-4 py-2">
            <div class="flex items-center mb-2">
              <img :src="cuser.photoURL" :alt="cuser.displayName" class="max-w-[50px] max-h-[50px] avatar" />
                ...
                <Menu>
                  <MenuButton
                    ...
                    <div class="flex flex-row space-x-2 items-center justify-center">
                      <img v-if="cprivacy.type === Privacy.PUBLIC" :src="menuItems[0].icon" class="w-4 h-4"/>
                      <img v-else-if="cprivacy.type === Privacy.FRIENDS" :src="menuItems[1].icon" class="w-4 h-4"/>
                      <img v-else-if="cprivacy.type === Privacy.PRIVATE" :src="menuItems[2].icon" class="w-4 h-4"/>
                      <span v-if="cprivacy.type === Privacy.PUBLIC"  class="font-sans font-semibold text-[13px] text-[#050505] w-full">{{ capitalize(menuItems[0].text) }}</span>
                      <span v-else-if="cprivacy.type === Privacy.FRIENDS"  class="font-sans font-semibold text-[13px] text-[#050505] w-full">{{ capitalize(menuItems[1].text) }}</span>
                      <span v-else-if="cprivacy.type === Privacy.PRIVATE" class="font-sans font-semibold text-[13px] text-[#050505] w-full">{{ capitalize(menuItems[2].text) }}</span>
                    </div>
                  ...
                  <Transition enter-active-class="transition duration-100 ease-out" enter-from-class="transform scale-95 opacity-0"
                    ...
                    <MenuItems
                      ...
                      <MenuItem v-slot="{ active }" v-for="(item, index) in menuItems" :key="index">
                        <button :class="[active ? 'bg-gray-200' : 'bg-white', 'group flex w-full items-center rounded-md px-2 py-2 text-sm text-gray-900 select-none space-x-4']" @click="() => setPrivacy(item.type as Privacy)">
            ...
            <div class="max-h-[400px] -z-10 overflow-y-scroll overflow-x-hidden nice-scrollbar">
              <textarea v-model="text" ref="textarea" v-focustrap
                ...
                placeholder="Bạn đang nghĩ gì?" cols="30" :rows="rows"></textarea>
              <div v-if="imageView" class="p-2 border border-gray-300 rounded-lg relative overflow-hidden">
                <i @click="clearImage()"
            ...
            <div class="bg-white">
              ...
              <div class="border-2 rounded-xl mt-4 shadow-sm flex items-center justify-between">
                ...
                <div class="flex items-center pr-2">
                  ...
                  <input id="image" accept="image/*" class="hidden" type="file" @input="getUploadedImage($event)" />
              ...
              <button @click="createPost" :disabled="disabledButton || error" type="button"
