      ...
      <div class="flex items-center justify-between pb-3 border-b">
        <div class="">
          ...
          >
            <AvatarGroup>
              <Avatar
                v-for="(name, index) in topReactions"
                :key="index"
                :image="getReaction(name)?.icon"
                shape="circle"
                :pt="{
                  image: 'w-5 h-5 ',
    ...
    <div class="px-5">
      ...
      >
        <div
          ...
          @mouseenter="showReaction = true"
          @mouseleave="showReaction = false"
        >
          <div
            class="absolute bottom-[calc(100%+10px)] z-[1] flex gap-1.5 rounded-full p-[6px] shadow w-[250px]"
            v-show="showReaction"
          >
            <div
              v-for="(reaction, index) in REACTIONS"
              :key="index"
              @click="updateReaction(reaction.name)"
          ...
          <div class="w-full h-full" @click="updateReaction(ReactionType.LIKE, true)">
            <div class="space-x-2 flex flex-row items-center justify-center">
              <img :src="getReaction(creaction)?.icon" class="w-[19px] h-[19px]" v-if="creaction" />
              <i v-else class="pi pi-thumbs-up w-[20px] h-[20px]" />
              <span
                class="text-sm"
                v-if="creaction"
                :style="{ color: getReaction(creaction)?.color }"
              >
