...
<template>
  <div class="w-full h-96 overflow-hidden relative select-none" v-if="user">
    <div class="min-w-0 flex flex-col m-0 min-h-0 items-stretch border-b border-white w-full h-48 overflow-hidden">
      <div v-if="props.user.coverURL"
        class="absolute w-full overflow-hidden top-0 overflow-y-hidden md:h-[222px] h-full block">
        <img :src="props.user.coverURL" class="inline-block w-full h-full object-cover overflow-clip text-ellipsis" />
      </div>
      <div v-if="!open" class="absolute left-1/2 -translate-x-1/2 -translate-y-1/2 top-1/2 max-w-xs">
        <Image v-if="photoImage" :src="photoImage" preview :pt="{
          image: {
      ...
      <router-link v-else :to="{ name: 'post', params: { id: id } }"
        class="absolute left-1/2 -translate-x-1/2 -translate-y-1/2 top-1/2 max-w-xs">
        <Image v-if="photoImage" :src="photoImage" :pt="{
          image: {
            class: 'rounded-full cursor-pointer border-4 border-white'
          },
          button: {
