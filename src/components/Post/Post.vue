...
<template>
  <Menu>
    <div class="w-full h-fit bg-white rounded-lg my-4 shadow-md" v-if="user">
        ...
        <div class="flex items-center justify-between p-2 rounded-full w-full">
          <div>
            <div class="font-medium text-[15px] text-[#050505] space-x-1">
              <router-link :to="{ name: 'user', params: { id: user.id } }" class="hover:underline">{{ user.displayName }}</router-link>
              <span class="font-light">{{ subnameInfo }}</span>
            </div>
            <div class="flex items-center justify-start gap-1 text-gray-600">
              <div class="flex items-center">
                <span class=" text-xs">{{ formatTimeAgo(new Date(post.created_at)) }}</span>
              </div>
              <img v-if="post.privacy === Privacy.PUBLIC" src="/icons/privacy/public.png" class="my-auto w-3 h-3" />
              <img v-else-if="post.privacy === Privacy.FRIENDS" src="/icons/privacy/friends.png" class="my-auto w-3 h-3" />
              <img v-else-if="post.privacy === Privacy.PRIVATE" src="/icons/privacy/private.png" class="my-auto w-3 h-3" />
            </div>
          </div>
            ...
            <Transition enter-active-class="transition duration-100 ease-out" enter-from-class="transform scale-95 opacity-0"
              ...
              <MenuItems
                ...
                <MenuItem v-slot="{ active }" v-for="(item, index) in menuItems" :key="index">
                  <button :class="[active ? 'bg-gray-200' : 'bg-white', 'group flex w-full items-center rounded-md px-2 py-2 text-sm text-gray-900 select-none space-x-4']" @click="item?.action">
      ...
      <PostProfilePhoto v-if="post.isProfilePhoto" :id="post.id" :open="open"
        :attachment="(post.attachment as IAttachmentItem)" :user="user" />
      <PostContent v-if="post.isNormalPost && post.attachment" :content="post.content" :id="post.id"
        :attachment="(post.attachment as IAttachmentItem)" :open="open" />
      <PostContent v-if="post.isNormalPost && !post.attachment" :content="post.content" :open="open" :id="post.id" />
      <PostContent v-if="post.isCoverPhoto" :attachment="(post.attachment as IAttachmentItem)" :open="open" :id="post.id" />
      <PostActionBar :pid="post.id" @toggle-comment="showComments = !showComments" :aid="post.uid" />
