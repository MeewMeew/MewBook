...
<template>
  <Menu as="div" class="flex w-full mb-1">
    ...
    <div class="flex flex-col justify-start items-start">
      <div class="flex items-center w-full relative">
        <div
          ...
          <div class="text-left [overflow-wrap:anywhere]" v-html="formatContent(comment.content)"></div>
        </div>
        ...
        <Transition enter-active-class="transition duration-100 ease-out" enter-from-class="transform scale-95 opacity-0"
          ...
          <MenuItems
            ...
            <MenuItem v-slot="{ active }" v-for="(item, index) in menuItems" :key="index">
              <button :class="[active ? 'bg-gray-200' : 'bg-white', 'group flex w-full items-center rounded-md px-2 py-2 text-sm text-gray-900 select-none']" @click="item.action">
      ...
      <div class="flex flex-row justify-start items-center pt-1 text-xs space-x-3 ml-2">
        <span class="hover:underline underline-offset-2 cursor-pointer font-bold text-[#65676B]">Thích</span>
        <span class="hover:underline underline-offset-2 cursor-pointer font-bold text-[#65676B]">Phản hồi</span>
